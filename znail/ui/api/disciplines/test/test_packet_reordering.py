...
class TestPacketReordering(unittest.TestCase):
    ...
    def test_can_be_set(self):
        response = self.client.post("/api/disciplines/packet_reordering", json={"milliseconds": 1, "percent": 2})
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json, {"message": "ok"})

        last_call = self.tc_apply.call_args_list[-1]
        self.assertEqual(last_call, call({"reorder": PacketReordering(milliseconds=1, percent=2.0)}))
    ...
    def test_can_be_updated(self):
        ...
        response = self.client.post("/api/disciplines/packet_reordering", json={"milliseconds": 2, "percent": 3})
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json, {"message": "ok"})

        last_call = self.tc_apply.call_args_list[-1]
        self.assertEqual(last_call, call({"reorder": PacketReordering(milliseconds=2, percent=3.0)}))
